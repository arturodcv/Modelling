 ...
 *    Memory model Changed to eliminate global workspaces; to allow parallelization
...
/* Computation of h * k_i; i >= 5. Normal cases: b_{i+1,2} = b_{i+1,3} = 0 */
        ...
        for(i=0; i < VField_dim; i++){ 
            ksub[10][i] = ksub[10][i] * *h; x7pred[i] = x[i] + beta[55] * ksub[0][i] + beta[60] * ksub[5][i] + beta[61] * ksub[6][i] + beta[62] * ksub[7][i] + beta                          [63] * ksub[8][i] + beta[64] * ksub[9][i]; }
        
        ODE_Sys(*t + alpha[11]* *h, x7pred, VField_dim, ksub[11], ParmsStruct);

        for(i=0; i < VField_dim; i++){ 
        ksub[11][i] = ksub[11][i] * *h; x7pred[i] = x[i] + beta[66] * ksub[0][i] + beta[69] * ksub[3][i] + beta[70] * ksub[4][i] + beta[71] * ksub[5][i] + beta[72]                       * ksub[6][i] + beta[73] * ksub[7][i] + beta[74] * ksub[8][i] + beta[75] * ksub[9][i] + ksub[11][i]; }
 ...
 *        c8[0] = c8[1] = c8[2] = c8[3] = c8[4] = c8[10] = 0   */
        *err = nor = 0.0;
        for(i=0; i < VField_dim; i++){ 
            ksub[12][i] = ksub[12][i] * *h;
            x7pred[i] = x[i] + c7[0] * ksub[0][i] + c7[5] * ksub[5][i] + c7[6] * ksub[6][i] + c7[7] * ksub[7][i] + c7[8] * ksub[8][i] + 
                        c7[9] * ksub[9][i] + c7[10] * ksub[10][i]; 
            x8pred[i] = x[i] + c8[5] * ksub[5][i] + c8[6] * ksub[6][i] + c8[7] * ksub[7][i] + c8[8] * ksub[8][i] + c8[9] * ksub[9][i] +
                        c8[11] * ksub[11][i] + c8[12] * ksub[12][i]; 
           // printf("x8pred = %f + %f + %f + %f + %f + %f +%f + %f \n",x[i], c8[5] * ksub[5][i], c8[6] * ksub[6][i], c8[7] * ksub[7][i], c8[8] * ksub[8][i], c8[9] * ksub[9][i], c8[11] * ksub[11][i] ,c8[12] * ksub[12][i]);
            nor += fabs(x8pred[i]); // L^1 norm of the prediction. Used to relativize tolerance
            *err += fabs(x8pred[i]-x7pred[i]);
        }
        
        if(isnan(nor) || isnan(*err)){ //printf("66\n");
          return 66;
        }
